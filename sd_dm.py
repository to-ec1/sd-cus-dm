...
# ── スクリプト自身の場所を基準に .env / キャッシュ / 認証ファイルを解決する ─────
# (313new.ps1経由ならSet-Location済みだが、他経路の呼び出しでも確実に同階層の.envを見るよう明示指定)
...
# 画像キャッシュディレクトリの定義（スクリプトと同階層）
CACHE_DIR = SCRIPT_DIR / "image_cache"
CACHE_DIR.mkdir(parents=True, exist_ok=True)

# ── 本ツール専用の隔離Chromeプロファイル ───────────────────────────
# ユーザーが普段使っている既存のChrome（デフォルトプロファイル・既存ウィンドウ）とは
# 完全に別物のプロファイル/ポートを使うことで、干渉（セッション混線・意図しないタブ操作等）を防ぐ。
CHROME_PROFILE_DIR = SCRIPT_DIR / ".chrome_profile_sd_dm"
CHROME_PROFILE_DIR.mkdir(parents=True, exist_ok=True)
CHROME_DEBUG_PORT = 9333  # 本ツール専用の固定デバッグポート
...
# ── 認証情報のパス（スクリプトと同階層。ローカル常設ファイル/GitHub Actions一時ファイル両対応） ──
CREDENTIAL_PATH = str(SCRIPT_DIR / "credentials.json")
TOKEN_PATH      = str(SCRIPT_DIR / "token.json")

# 環境変数(GitHub Actions Secrets)からGoogle認証ファイルを生成した場合のみ、
# 終了時にそのファイルを削除する。ローカルに既存の credentials.json/token.json は削除しない。
_CRED_FILE_CREATED_BY_ENV = False
_TOKEN_FILE_CREATED_BY_ENV = False
...
def try_launch_chrome(headless=False):
    """5パターンの起動施策を検証。完全に人間的なフィンガープリントへの偽装処理を統合。
    ※全パターン共通で本ツール専用の隔離プロファイル(CHROME_PROFILE_DIR)と
      専用デバッグポート(CHROME_DEBUG_PORT)を使用する。これにより、
      ユーザーが普段使っている既存のChromeウィンドウ・デフォルトプロファイルには
      一切干渉せず、完全に別プロセス・別セッションとして分離される。"""
    UA_DESKTOP = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36"

    strategies = [
        {
            "name": "施策1: 新ヘッドレス/有頭 + 完全検出回避パラメータ",
            "setup": lambda co: [
                co.set_argument('--headless=new') if headless else None,
                co.set_argument('--no-sandbox'),
                co.set_argument('--disable-gpu'),
                co.set_argument('--disable-dev-shm-usage'),
                co.set_argument('--remote-allow-origins=*'),
                co.set_argument(f'--remote-debugging-port={CHROME_DEBUG_PORT}'),
                co.set_argument(f'--user-data-dir={CHROME_PROFILE_DIR}'),
        ...
        {
            "name": "施策2: DP推奨 headless設定 + 完全検出回避パラメータ",
            "setup": lambda co: [
        ...
        },
        {
            ...
            "setup": lambda co: [
                co.set_argument('--headless') if headless else None,
                co.set_argument('--no-sandbox'),
                co.set_argument('--disable-gpu'),
                co.set_argument('--disable-dev-shm-usage'),
                co.set_argument('--remote-allow-origins=*'),
...
def main():
    ...
    parser.add_argument('--headless', action='store_true', help='ヘッドレスモードで起動する場合に指定（指定なしの場合、ローカル画面表示の有頭モード）')
    ...
    # 🌟 テスト開発モード: デフォルトで画面表示 (headless=False)
    use_headless = True if args.headless else False
    print(f"🖥️ ブラウザ起動モード: {'ヘッドレス（非表示）' if use_headless else '有頭（画面表示・目視可能）'}")
    page = try_launch_chrome(headless=use_headless)
    ...
    try:
        ...
        if page:
            try:
                ...
                pass
        # ローカルの常設 credentials.json / token.json は削除しない。
        # GitHub Actions Secrets からこの実行のために書き出した場合のみ後始末として削除する。
        _cleanup_targets = []
        if _CRED_FILE_CREATED_BY_ENV:
            _cleanup_targets.append(CREDENTIAL_PATH)
        if _TOKEN_FILE_CREATED_BY_ENV:
            _cleanup_targets.append(TOKEN_PATH)
